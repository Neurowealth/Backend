...
import { validateScopesInput, ADMIN_SCOPES, AdminScope } from '../middleware/adminAuth';
...
function makeRes(scopes: AdminScope[]) {
  const locals: any = { adminAuth: { id: 'key-1', name: 'test-key', role: 'admin', scopes } };
  const res: any = {
    locals,
...
function makeReq(): Request {
  return { ip: '127.0.0.1', headers: {}, originalUrl: '/admin/test', method: 'GET' } as any;
}
...
describe('requireAdminScope middleware', () => {
  ...
  test('blocks request and returns 403 when scope missing', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining({ success: false, error: "Admin scope 'write' required" }),
    );
  });
  ...
  test.each([
    ...
    ['agent',  ['super'],                  true],
    ['write',  ['super'],                  true],
  ])('scope=%s, grantedScopes=%j → allowed=%s', async (required, granted, allowed) => {
