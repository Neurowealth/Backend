...
describe('calculateYearsRemaining', () => {
  it('is positive for a future date and negative for a past date', () => {
    ...
    expect(
      calculateYearsRemaining(new Date('2027-01-01T00:00:00Z'), from)
    ).toBeGreaterThan(0.9)
    expect(
      calculateYearsRemaining(new Date('2025-01-01T00:00:00Z'), from)
    ).toBeLessThan(0)
  })
})
...
describe('GoalTrackingStrategy', () => {
  ...
  it('unreachable-within-risk-ceiling: surfaces target-not-reachable instead of exceeding the ceiling', async () => {
    ...
    expect(decision.reasoning).toContain(
      'not reachable within your risk tolerance'
    )
