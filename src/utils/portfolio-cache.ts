...
const PORTFOLIO_CACHE_TTL = parseInt(process.env.PORTFOLIO_CACHE_TTL_SECONDS || '60');

export function portfolioCacheKey(userId: string): string {
  return `portfolio_snapshot:${userId}`;
}

export async function getPortfolioSnapshot<T>(userId: string): Promise<T | null> {
  return cacheGet<T>(portfolioCacheKey(userId));
}

export async function setPortfolioSnapshot(userId: string, data: unknown): Promise<void> {
 ...
 */
export async function invalidatePortfolioSnapshot(userId: string): Promise<void> {
