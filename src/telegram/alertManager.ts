...
/**
 * Telegram-facing alert-rule management (#402).
 *
 * The Telegram layer identifies users by wallet address (the in-memory chat
 ...
 * — the exact pattern src/whatsapp/alertManager.ts uses. A rule is only ever
 * visible or mutable by its owner: the walletAddress → userId resolution IS
 * the ownership check here.
 */
...
export async function createAlertRuleForWallet(
  ...
  if (!parsed.success) {
    // Surface a single friendly hint rather than raw Zod detail over Telegram.
    return {
      ok: false,
      error:
